...
export default function App() {
  const [step, setStep] = useState(0); const [answers, setAnswers] = useState({}); const [showResults, setShowResults] = useState(false); const [fadeIn, setFadeIn] = useState(true); const [showConfig, setShowConfig] = useState(false); const [copied, setCopied] = useState(false);
            ...
            <div>
              ...
              <div style={{ marginTop: 28, textAlign: "center" }}>
                <div onClick={() => {
                  ...
                  const prompt = `Create an Antigravity workspace for my project using ${fw.name}.\n\nProject: ${answers.problem || "Untitled"}\nUser: ${answers.user || ""}\nFramework: ${fw.name} — ${fw.why}\n\n## Execution Pipeline\n\n${pipeline}\n\n## Advisory Agents\n\n${specs}`;
                  if (typeof sendPrompt === "function") { sendPrompt(prompt); }
                  else { navigator.clipboard?.writeText(prompt).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000); }); }
                }} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "12px 28px", background: copied ? "#4CAF50" : "#E8945A", color: "#1A1714", borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: "pointer", boxShadow: copied ? "0 2px 12px rgba(76,175,80,0.3)" : "0 2px 12px rgba(232,148,90,0.25)", transition: "background 0.2s,box-shadow 0.2s" }}
                  onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-1px)"; }} onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; }}>
                  {copied
                    ? <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M2 8l4 4 8-8" stroke="#1A1714" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
                    : <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><rect x="5" y="2" width="9" height="11" rx="1.5" stroke="#1A1714" strokeWidth="1.6" /><path d="M3 5H2.5A1.5 1.5 0 001 6.5v7A1.5 1.5 0 002.5 15h6A1.5 1.5 0 0010 13.5V13" stroke="#1A1714" strokeWidth="1.6" strokeLinecap="round" /></svg>
                  }
                  {copied ? "Copied to clipboard!" : "Copy Architecture Spec"}
                </div>
                <div style={{ fontSize: 11, color: "#5A564F", marginTop: 10 }}>Paste into Claude or Antigravity to scaffold your project</div>
              </div>
            </div>
          )}
        </div>
